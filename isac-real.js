...
function bindEvents() {
  ...
  document.querySelectorAll("[data-profile]").forEach((button) => {
    button.addEventListener("click", () => {
      state.profileKey = button.dataset.profile;
      addIsacEvent(`码率切换：${COMM_PROFILES[state.profileKey].label}`);
      renderCommunicationPanel();
  ...
  document.querySelectorAll("[data-channel]").forEach((button) => {
    button.addEventListener("click", () => {
      state.channelKey = button.dataset.channel;
      addIsacEvent(`信道切换：${COMM_CHANNELS[state.channelKey].label}`);
      renderCommunicationPanel();
      publishRemoteState({ force: true });
    });
  });
...
async function stopMonitoring() {
  ...
  if (wasRunning) {
    renderIdle("监测已停止。点击连接 ESP32 并开始监测可重新建立电磁环境投影。");
  } else {
    renderIdle();
  }
}
...
function buildResultText(result, paper, changeHit, paperHit, present) {
  if (present) {
    if (changeHit && paperHit) {
      return "鲁棒 CSI 投影门控与 Doppler-NMI 图门控均连续触发，判定为有人活动。";
    }
    if (paperHit) {
      return "Doppler-NMI 图门控连续触发，判定为有人活动。";
    }
    return "鲁棒 CSI 投影门控连续触发，判定为有人活动。";
  }
  if (result.artifactReason) {
    return `${result.artifactReason}，暂不判定为有效人体活动。`;
  }
  if (changeHit || paperHit) {
    return "已有单门控候选响应，正在等待时间持续性确认。";
  }
  if (!paper.available) {
    return "正在积累 Doppler-NMI 图确认所需的 CSI 窗口。";
  }
  return "CSI 动态特征接近电磁环境投影，未发现持续人体活动。";
}
...
function renderIdle(message = "点击连接 ESP32 并开始监测。先给 5 秒准备时间，随后 10 秒保持空场稳定并建立电磁环境投影，同时观察通信业务流指标。") {
  setStateLabel("待初始化", "idle");
  ...
  els.mainButton.textContent = "连接 ESP32 并开始监测";
  els.mainButton.classList.remove("stop");
  els.mainButton.disabled = false;
  if (!hasWebSerial()) {
    setSerialStatus("当前浏览器不支持串口", "warn");
    els.mainButton.textContent = "请用 Mac Chrome 打开";
    els.mainButton.disabled = true;
    els.presenceText.textContent =
      "当前浏览器不能直接连接 ESP32。请在 Mac 的 Chrome 或 Edge 打开本页，再点击连接 ESP32 并开始监测；手机端只负责远程查看结果。";
  }
  renderCommunicationPanel();
  renderIsacRows();
...
function renderBaseline() {
  ...
  els.presenceText.textContent =
    remain > 0
      ? `正在建立电磁环境投影，还剩 ${remain} 秒。此时尽量保持链路周围无人移动。`
      : "投影样本不足，继续等待 CSI 帧。";
  els.presenceScore.textContent = "0";
  els.scoreFill.style.width = "0%";
...
function renderLayerCardsIdle() {
  setLayerCard("change", "idle", "待初始化", "归一化 CSI 形态漂移检测", 0, "--");
  setLayerCard("paper", "idle", "待初始化", "Doppler-NMI 图与 QAOA 分区映射", 0, "--");
  setLayerCard("combined", "idle", "待初始化", "双门控 OR 融合与时间持续性判决", 0, "连续确认");
}

function renderLayerCardsBaseline(remain) {
  const text = remain > 0 ? `还剩 ${remain} 秒` : "等待更多 CSI 帧";
  setLayerCard("change", "wait", "投影建模", text, 0, "--");
  setLayerCard("paper", "wait", "图特征预热", text, 0, "--");
...
function renderLayerCardsMonitoring(result) {
  ...
  if (result.changeHit) {
    ...
    setLayerCard("change", "wait", "伪迹过滤", result.artifactReason, changeScore, changeThreshold);
  } else if (result.artifactReason === "数据不足") {
    setLayerCard("change", "wait", "窗口不足", "实时窗口 CSI 帧不足", changeScore, changeThreshold);
  } else {
    setLayerCard("change", "clear", "投影一致", "CSI 形态接近电磁环境投影", changeScore, changeThreshold);
  }

  if (!paperAvailable) {
    setLayerCard("paper", "wait", "图窗口不足", "Doppler-NMI 需要更长 CSI 窗口", paperScore, paperThreshold || "--");
  } else if (result.paperHit) {
    setLayerCard("paper", "hit", "图门控触发", "检测到非零 Doppler 动态分量", paperScore, paperThreshold);
  } else {
    setLayerCard("paper", "clear", "图响应稳定", "未发现持续 Doppler 动态分量", paperScore, paperThreshold);
  }

  if (result.present) {
    setLayerCard("combined", "hit", "有人活动", "任一门控持续触发后提醒", combinedScore, "连续确认");
  } else if (result.changeHit || result.paperHit) {
    setLayerCard("combined", "wait", "等待确认", "单门控响应，等待持续性确认", combinedScore, "连续确认");
  } else {
    setLayerCard("combined", "clear", "无人活动", "双门控均未连续触发", combinedScore, "连续确认");
  }
}
...
function setRunningButton() {
  els.mainButton.disabled = false;
  els.mainButton.textContent = "停止监测与业务流";
...
function startCommunicationStream() {
  resetCommunicationStats();
  state.commRunning = true;
  addIsacEvent("业务流启动");
  renderCommunicationPanel();
}

function stopCommunicationStream() {
  if (state.commRunning) addIsacEvent("业务流暂停");
  state.commRunning = false;
  renderCommunicationPanel();
}

function resetCommunicationStats() {
  state.lastFrameAt = 0;
  state.lastDrawAt = 0;
  state.frameSeq = 0;
  state.txFrames = 0;
...
function commLoop(now) {
  if (state.commRunning) {
    const profile = COMM_PROFILES[state.profileKey];
    const frameInterval = 1000 / profile.fps;
    if (now - state.lastFrameAt >= frameInterval) {
      state.lastFrameAt = now;
      simulateCommFrame(now);
    }
  }

  if (now - state.lastDrawAt >= 33) {
    state.lastDrawAt = now;
    drawCommunicationCanvas(now);
  }

  requestAnimationFrame(commLoop);
}

function simulateCommFrame(now) {
  const profile = COMM_PROFILES[state.profileKey];
  const channel = COMM_CHANNELS[state.channelKey];
  state.txFrames += 1;
  state.frameSeq += 1;

  if (Math.random() < channel.loss) {
  ...
  state.deliveredBytes.push({ time: now, bytes });
  state.deliveredFrames.push({ time: now });
  state.latencies.push(latency);
  trimCommMetrics(now);
}

function trimCommMetrics(now) {
  while (state.deliveredBytes.length && now - state.deliveredBytes[0].time > 1000) state.deliveredBytes.shift();
  while (state.deliveredFrames.length && now - state.deliveredFrames[0].time > 1000) state.deliveredFrames.shift();
  while (state.latencies.length > 60) state.latencies.shift();
}

function computeCommMetrics() {
  const byteSum = state.deliveredBytes.reduce((sum, item) => sum + item.bytes, 0);
  const throughput = (byteSum * 8) / 1_000_000;
  const fps = state.deliveredFrames.length;
  const latency = mean(state.latencies);
  const jitter = std(state.latencies);
  const lossRate = state.txFrames ? (state.droppedFrames / state.txFrames) * 100 : 0;
  const linkScore = Math.round(
    clamp(100 - lossRate * 2.2 - Math.max(0, latency - 35) * 0.22 - jitter * 0.35, 0, 100),
  );
  return {
    ...
    linkScore: Number.isFinite(linkScore) ? linkScore : 0,
  };
}

function renderCommunicationPanel() {
  const metrics = computeCommMetrics();
  const profile = COMM_PROFILES[state.profileKey];
  const channel = COMM_CHANNELS[state.channelKey];

  if (els.linkStatus) {
    els.linkStatus.textContent = state.commRunning ? `${profile.label} · ${channel.label}` : "业务流暂停";
    els.linkStatus.className = `status-pill ${state.commRunning ? "ok" : ""}`.trim();
  }
  if (els.streamState) els.streamState.textContent = state.commRunning ? "业务流传输中" : "业务流暂停";
  if (els.frameId) els.frameId.textContent = `Frame ${String(state.frameSeq).padStart(4, "0")}`;
  if (els.throughput) els.throughput.textContent = metrics.throughput.toFixed(2);
  if (els.fps) els.fps.textContent = metrics.fps.toFixed(1);
  if (els.latency) els.latency.textContent = String(Math.round(metrics.latency));
  if (els.jitter) els.jitter.textContent = String(Math.round(metrics.jitter));
  if (els.lossRate) els.lossRate.textContent = metrics.lossRate.toFixed(1);
  if (els.linkScore) els.linkScore.textContent = String(metrics.linkScore);
  renderIsacRows();
}

function drawCommunicationCanvas(now) {
  ...
  const label = els.presenceState?.textContent || "待初始化";
  const active = state.latestResult?.present || score >= 60;
  const weak = !active && score >= 35;
  const t = now / 1000;

  ctx.clearRect(0, 0, width, height);
  const gradient = ctx.createLinearGradient(0, 0, width, height);
  gradient.addColorStop(0, "#17243a");
  gradient.addColorStop(0.48, channel === COMM_CHANNELS.fading ? "#5b2333" : "#1f6fd2");
  ...
  const radius = active ? 54 : weak ? 44 : 36;
  const orbX = width * 0.52 + Math.sin(t * 1.8) * width * 0.22;
  const orbY = height * 0.52 + Math.cos(t * 1.4) * height * 0.17;
  ctx.beginPath();
  ctx.arc(orbX, orbY, radius, 0, Math.PI * 2);
  ctx.fillStyle = active ? "#ff7369" : weak ? "#ffd95a" : "#8ee2ae";
  ctx.fill();
  ctx.lineWidth = 6;
  ...
  ctx.fillText(`CSI ${label}`, 24, 92);
  ctx.font = "600 18px system-ui";
  ctx.fillText(`Frame ${String(state.frameSeq).padStart(4, "0")} · RX ${state.rxFrames} / TX ${state.txFrames}`, 24, height - 30);
  ...
  if (!state.commRunning) {
    ...
    ctx.fillText("等待 ESP32 CSI 连接", width / 2 - 180, height / 2 + 14);
  }
}

function addIsacEvent(text) {
  const metrics = computeCommMetrics();
  state.eventRows.unshift({
    time: new Date().toLocaleTimeString("zh-CN", { hour12: false }),
    event: text,
    comm: `${metrics.throughput.toFixed(2)} Mbps / ${metrics.lossRate.toFixed(1)}% loss`,
    sensing: els.presenceState?.textContent || "待初始化",
  });
  state.eventRows = state.eventRows.slice(0, 8);
  renderIsacRows();
}

function updateSensingEvent(result) {
  const label = result.present
    ? "融合判定：有人活动"
    : result.changeHit || result.paperHit
      ? "候选人体活动"
      : result.artifactReason && result.artifactReason !== "数据不足"
        ? `伪迹过滤：${result.artifactReason}`
        : "电磁环境稳定";
  if (label === state.lastPresenceEvent) return;
  state.lastPresenceEvent = label;
  addIsacEvent(label);
}

function renderIsacRows() {
  if (!els.eventRows) return;
  if (!state.eventRows.length) {
    els.eventRows.innerHTML = '<tr><td colspan="4">连接 ESP32 后记录业务流和感知事件</td></tr>';
    return;
  }
  els.eventRows.innerHTML = state.eventRows
...
function buildRemoteStatePayload(now) {
  const score = Number(els.presenceScore?.textContent || 0);
  const comm = computeCommMetrics();
  const profile = COMM_PROFILES[state.profileKey];
  const channel = COMM_CHANNELS[state.channelKey];
  return {
    phase: state.phase,
    source: "isac-real-page",
    localTime: now,
    ...
    isac: {
      running: state.commRunning,
      profile: profile.label,
      channel: channel.label,
      throughputMbps: comm.throughput,
      fps: comm.fps,
      latencyMs: comm.latency,
      jitterMs: comm.jitter,
      lossRate: comm.lossRate,
      linkScore: comm.linkScore,
      txFrames: state.txFrames,
      rxFrames: state.rxFrames,
      csiPackets: state.totalPackets,
    },
  };
}
